...
To install divo package first type \texttt{install.packages("divo")} and load it into an R session by typing \texttt{library(divo)}. To get help type \texttt{?divo}.
...
To calculate similarity index of diversity profile, first load data as data frame of populations (columns) and species (rows) first and then select a test. For example to resample input data set 50 times and run I-index:
