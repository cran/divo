...
void DP_single_population_HT_cvg(int* ptrCol,int* ptrdimAfa,double* ptrAlphaPro,int nAlphaPro,int ENS,double valOutCvg,double* ptrResults){
	...
	//Only change compared to DP_single_population_HT.c is "ptrAlphaPro[i]*valOutCvg" in function DP_HT()
	for (int i = 0; i < nAlphaPro; i++){
		ptrResults[i]=DP_HT(ptrCol_P,nCol_P,ptrAlphaPro[i] * valOutCvg,ENS,Sum);
	}	
}
